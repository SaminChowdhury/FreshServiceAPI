...
while x ==0:
    ...
    def sortData(ticketArray):
        ...
        for ticket in ticketArray:
                ...
                #VIP
                    try:
                        vip = requester_data["requester"]["vip_user"]
                        if (vip=="true"):
                            vips.append('true')
                        else:
                            vips.append("false")
    ...
    df = pd.DataFrame({'Requester':Requesters,'TicketID':TicketID,'Summary':Sub_Line,'Building':Building,'AgentName':Agents,'Priority':priorities,'VIP':vips})
    df.to_csv('/home/freshservice/SystemAdmin/directory.csv', index=False, encoding='utf-8')
    print("\n" + str(datetime.datetime.now()) + " CSV done!")
    try:
        os.popen('sh /home/freshservice/SystemAdmin/scp.sh')
    except:
        time.sleep(1)
